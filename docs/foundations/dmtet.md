...
传统 Marching Cubes 使用规则的立方体网格，DMTet 的关键创新是：
...
四面体的最大优势是**拓扑无歧义**：一个四面体只有 4 个顶点，$2^4 = 16$ 种内外组合，归约后仅剩 3 种基本情况（无面片、一个三角形、两个三角形），不存在 MC 中多种等价切割方式的歧义问题。
...
DMTet 在 SDS 驱动的 Text-to-3D 中的代表性应用：
