...
这种分解避免了直接建模"编辑"这个复杂操作，转而将其还原为两个定义清晰、各自最优的子问题。
...
这种自监督策略巧妙地规避了 3D 编辑领域最大的瓶颈之一——缺乏大规模高质量编辑数据对。
...
Vertex alignment 策略不仅带来了推理加速，还显著提升了生成质量。这说明让模型在训练时就"意识到"三个坐标之间的关联，有助于学到更好的几何表示。
...
- **原子操作分解**：将复杂编辑还原为删除 + 添加两个清晰子问题，设计优雅
...
MeshPad 的核心贡献是提出将 Mesh 编辑分解为删除和添加两个原子操作，在 MeshAnythingV2 自回归框架上实现了草图驱动的交互式 Mesh 编辑，并通过 Vertex-Aligned Speculator 将推理速度提升到交互级别。
