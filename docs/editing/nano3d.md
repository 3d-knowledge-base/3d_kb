...
**核心思想**：通过 XOR 运算找到源和目标之间的所有差异体素，再通过连通域分析 + 体积阈值过滤掉因扩散过程引入的零散噪声，只保留真正有意义的编辑区域。最终得到的 flip mask $M$ 精确定义了"哪些体素需要改变"。

**阈值 $\tau$**：控制连通域的最小体积。$\tau = 100$ 为最优值——太小会引入噪声碎片，太大会过滤掉合理的小编辑。
...
DDIM 反演的根本问题在于：反演过程本身会引入累积误差，导致重建出的噪声潜变量无法完美还原源数据。FlowEdit 绕过了这个问题：
...
**95% 的用户偏好** NANO3D 的形状保持效果，说明 Voxel-Merge + Slat-Merge 的显式融合策略在人类感知层面也显著优于基线方法。
...
$\tau = 100$ 为最优阈值。低于 100 时噪声碎片影响融合质量，高于 100 时合理的小编辑区域被误过滤。
...
NANO3D 在 TRELLIS 骨干上走出了一条与 VoxHammer 不同的 training-free 编辑路线：用 FlowEdit 替代 DDIM 反演避免累积误差，用 Voxel-Merge + Slat-Merge 的双层显式融合替代注意力层面的 KV Cache 替换。其核心价值不仅在于编辑方法本身，还在于 Nano3D-Edit-100k 数据集的构建——这一数据集为后续监督式 3D 编辑方法（如 3DEditVerse、Steer3D 等）提供了可能的训练数据来源，推动了整个 3D 编辑领域从 tuning-free tricks 向 data-driven learning 的转变。
