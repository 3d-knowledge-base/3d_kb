...
这本质上是将 P2P（Prompt-to-Prompt）系列方法的 attention manipulation 思想，从 2D 扩散模型扩展到 TRELLIS 的 3D 稀疏 Flow Transformer。
...
这确保了未编辑区域的 token 在注意力计算中"看到的"与原始资产完全一致，从根本上阻断了编辑信号的泄漏路径。
...
| 配置 | 效果 |
|:-----|:-----|
| 移除 KV Cache Replacement | 保留度**显著下降**，编辑泄漏明显 |
...
VoxHammer 的核心贡献是证明了"DDIM 反演 + KV Cache 替换"这套 2D 扩散编辑范式可以直接迁移到 TRELLIS 的 3D 结构化潜空间中，在不训练任何额外参数的前提下实现高保真的局部 3D 编辑。
