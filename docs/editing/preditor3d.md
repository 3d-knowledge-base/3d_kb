...
大多数 3D 编辑方法面临一个根本性矛盾：**编辑区域的质量**和**非编辑区域的保持**很难两全。如果只重建编辑后的结果，非编辑区域不可避免地会发生漂移；如果强行约束非编辑区域，编辑的效果又会打折扣。

PrEditor3D 的解法非常直觉：

> 既然一条路不够好，就走两条路——分别重建原始版本和编辑版本的**完整** 3D 特征网格，然后从原始版本取"完美背景"，从编辑版本取"完美前景"，在特征空间拼接融合。

这样，$V_i$（原始特征网格）拥有完美的未编辑区域，$V_e$（编辑特征网格）拥有完美的编辑结果。两者各取所长，合并为最终输出。
...
这是 PrEditor3D 最核心的阶段，也是"两路并进"思想的具体实现。
...
核心洞察在于：**$V_i$ 的未编辑区域是完美的，$V_e$ 的编辑区域是完美的**。两者都是完整重建的特征网格，不存在局部缺失。
...
PrEditor3D 的消融实验非常清晰地验证了 pipeline 每一环的必要性：
...
CLIPdiff-noedit 指标（越低越好）：**2.636**，在所有对比方法中表现最佳，证明了"两路并进"策略对非编辑区域保持的有效性。
